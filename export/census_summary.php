<?php
	...
	$query = $mydb->dbquery("SELECT a.code, a.procedure, c.subcategory AS subcatname, COUNT(a.code) AS testcount FROM lab_samples a LEFT JOIN services_master b ON a.code = b.ihomis_code LEFT JOIN options_servicesubcat c ON b.subcategory = c.id WHERE a.extractdate BETWEEN '".$mydb->formatDate($_GET['dtf'])."' AND '".$mydb->formatDate($_GET['dt2'])."' AND a.status IN (3,4) $f1 GROUP BY a.code ORDER BY c.subcategory, a.procedure;");
	//echo "SELECT a.code, a.procedure, c.subcategory AS subcatname, COUNT(a.code) AS testcount FROM cph_carcar.lab_samples a LEFT JOIN cph_carcar.services_master b ON a.code = b.ihomis_code LEFT JOIN cph_carcar.options_servicesubcat c ON b.subcategory = c.id WHERE a.extractdate BETWEEN '".$mydb->formatDate($_GET['dtf'])."' AND '".$mydb->formatDate($_GET['dt2'])."' AND a.status IN (3,4) $f1 GROUP BY a.code ORDER BY c.subcategory, a.procedure;";
	
	//$query = $mydb->dbquery("SELECT distinct a.code, a.procedure, COUNT(a.procedure) from lab_samples a where a.extractdate between '2023-05-01' and '2023-05-31' group by a.code;");
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("A1","CEBU PROVINCIAL HOSPITAL - CARCAR CITY");
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("A2","Baracca, Poblacion II, Carcar City, Cebu");
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("A3","(32) 887-3292 / 501-0427");
