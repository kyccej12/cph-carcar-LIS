...
/* MYSQL QUERIES SECTION */
    ...
    $b = $o->getArray("select * from lab_dengueduo where enccode = '$order[enccode]' and serialno = '$order[serialno]' and `code` = '$order[code]';");
    ...
    //list($room) = $o->getArray("SELECT concat(b.wardcode,'-',rmname,'-',bdname) FROM cph_carcar.lab_samples a LEFT JOIN hospital_dbo.hpatroom b ON a.enccode = b.enccode LEFT JOIN hospital_dbo.hroom c ON b.rmintkey = c.rmintkey LEFT JOIN hospital_dbo.hbed d ON b.bdintkey = d.bdintkey WHERE a.enccode = '$a[enccode]' limit 1;");
...
$mpdf=new mPDF('win-1252','LETTER','','',5,5,95,5,5,10);
...
<table width=100% cellpadding=2 cellspacing=0 style="font-size: 10pt;margin-top:20px;">
	...
	<tr>
	<td width=100% colspan=4 style="background-color: #8db4e2; border-top: 1px solid black; padding: 5px; font-weight: bold;" align=center>DENGUE QUALITIVE TEST</td>
	</tr>  
</table>
</htmlpageheader>
...
mpdf-->
<div id="main">
        <table width=60% cellpadding=0 cellspacing=0 align=center style="border-collapse: collapse; font-size: 9pt;">
        <tr>
            <td class="itemHeader" align=left>TEST</td>
            <td class="itemHeader">RESULT</td>
        </tr>';

        if($b['result'] != '') {
            $html .= '<tr>
                <td class="itemResult" align=left>Dengue NS1 Antigen</td>
                <td class="itemResult"><b>'.$b['result'].'</b></td>
            </tr>';
        }
        if($b['result2'] != '') {
            $html .= '<tr>
                <td class="itemResult" align=left>Dengue IgG Antibody</td>
                <td class="itemResult"><b>'.$b['result2'].'</b></td>
            </tr>';
        }
        if($b['result2'] != '') {
            $html .= '<tr>
                <td class="itemResult" align=left>Dengue IgM Antibody</td>
                <td class="itemResult"><b>'.$b['result3'].'</b></td>
            </tr>';
        }
    $html .= '</table>
    <table width=70% align=center style="margin-top: 5px; font-size: 9pt; font-style: italic;">
        <tr>
