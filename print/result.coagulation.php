...
/* MYSQL QUERIES SECTION */
	...
	if($b['proth_seconds'] > 24) { $flag1 = 'H'; }
	if($b['proth_seconds'] < 16) { $flag1 = 'L'; }
	...
	if($b['aptt_seconds'] < 43.8) { $flag4 = 'L'; }
	if($b['aptt_seconds'] > 65.7) { $flag4 = 'H'; }
...
<table width=95% cellpadding=0 cellspacing=0 align=center font-size: 10pt;>
    ...
    if(checkTest('L146',$order['serialno'])) {
        ...
        <tr>
            ...
            <td align=center>'.$con->checkChemValues($con->age,$a['xgender'],'PROTH_SEC',$b['proth_seconds']).'</td>
            <td align=center valign=top>'.$con->getAttribute('PROTH_SEC',$con->age,$a['xgender']).'</td>	
            <td align=center>seconds</td>	
        </tr>';
        if($b['proth_percent'] > 0) {
        $html .= '
            <tr><td colspan=4 height=5>&nbsp;</td></tr>
            <tr>
                <td align=center style="padding-left: 25px;"></td>
                <td align=center>'. $b['proth_percent'] . '</td>
                <td align=center>'.$con->checkChemValues($con->age,$a['xgender'],'PROTH_PERCENT',$b['proth_percent']).'</td>
                <td align=center valign=top>'.$con->getAttribute('PROTH_PERCENT',$con->age,$a['xgender']).'</td>	
                <td align=center>%</td>	
            </tr>';
        }
        $html .= '
        ...
        <tr>
            ...
            <td align=center>'.$con->checkChemValues($con->age,$a['xgender'],'PROTH_INR',$b['proth_inr']).'</td>
            <td align=center valign=top>'.$con->getAttribute('PROTH_INR',$con->age,$a['xgender']).'</td>	
    ...
    if(checkTest('L050',$order['serialno'])) {
        $html .= '<tr>
            ...
            <td align=center>'.$con->checkChemValues($con->age,$a['xgender'],'APTT_SEC',$b['aptt_seconds']).'</td>
            <td align=center valign=top>'.$con->getAttribute('APTT_SEC',$con->age,$a['xgender']).'</td>	
            <td align=center>seconds</td>	
        </tr>
