...
/* MYSQL QUERIES SECTION */
    ...
    $a = $con->getArray("SELECT docointkey, a.enccode, SUBSTR(enccode,8,15) AS hmrno, DATE_FORMAT(dodate,'%m/%d/%Y %h:%i %p') AS orderdate, DATE_FORMAT(dodate,'%Y-%m-%d') AS xorderdate, a.hpercode, concat(URLENCODE(c.patlast),', ', URLENCODE(c.patfirst),', ', URLENCODE(ifnull(c.patmiddle,''))) as pname,  DATE_FORMAT(c.patbdate,'%m/%d/%Y') AS bday, DATE_FORMAT(c.patbdate,'%Y-%m-%d') AS xbday, IF(c.patsex='F','FEMALE','MALE') AS sex, c.patsex as gender, a.proccode, b.procdesc, a.donotes AS remarks, a.licno, a.estatus, entby FROM hospital_dbo.hdocord a LEFT JOIN hospital_dbo.hprocm b ON a.proccode = b.proccode LEFT JOIN hospital_dbo.hperson c ON a.hpercode = c.hpercode WHERE a.enccode = '$order[enccode]';");
    $b = $con->getArray("select * from lab_singleresult where enccode = '$order[enccode]' and `code` = '$order[code]' and serialno = '$order[serialno]';");
	...
	//list($room) = $con->getArray("SELECT concat(b.wardcode,'-',rmname,'-',bdname) FROM cph_carcar.lab_samples a LEFT JOIN hospital_dbo.hpatroom b ON a.enccode = b.enccode LEFT JOIN hospital_dbo.hroom c ON b.rmintkey = c.rmintkey LEFT JOIN hospital_dbo.hbed d ON b.bdintkey = d.bdintkey WHERE a.enccode = '$a[enccode]' limit 1;");
	$room = $con->identRoom($a['enccode']);
	if($room == '') { 
        list($room) = $con->getArray("select toecode from hospital_dbo.henctr where enccode = '$order[enccode]';");
    }

	if($order['primecarecode'] == 'L113') {
			$attr = "70 - 115";
			if($b['value'] <= 69.9) {
				$flag = "<font color=red><b>L</b></font>";
			}

			// if($b['value'] > 100 && $b['value'] < 126) {
			// 	$attr = "70 -115";
			// }
			
			if($b['value'] > 115.1) {
				$flag = "<font color=red><b>H</b></font>";
			}
	} else {
		$attr = $con->getAttribute($order['primecarecode'],$con->age,$a['gender']);
	}

/* END OF SQL QUERIES */

$mpdf=new mPDF('win-1252','LETTER','','',10,10,68,30,5,10);
...
<head>
	<style>
        ...
        .itemHeader {
            padding:5px;border:1px solid black; text-align: center; font-weight: bold; border-left:none; border-right:none;
        }
...
<htmlpagefooter name="myfooter">
    <table width=100% cellpadding=5 style="margin-bottom: 5px;">
        ...
        <tr>
			<td align=center>'.$encSignature.'<br/><b>'.$encBy.'&nbsp;<br/>_____________________________<br/><span style="font-size: 8pt;">PRC LICENSE NO. '.$encByLicense.'</span><br/><b>REPORTED BY</b></td>
            <td align=center>'.$cbySignature.'<br/><b>'.$cby.'&nbsp;<br/>_____________________________<br/><span style="font-size: 8pt;">PRC LICENSE NO. '.$cbyLicense.'</span><br/><b>VALIDATED BY</b></td>
            <td align=center valign=top><img src="../images/signatures/psa-signature.png" align=absmidddle /><br/><b>PETER S. AZNAR, M.D, F.P.S.P<br/>_____________________________<br><span style="font-size: 8pt;">PRC LICENSE NO. 72410</span><br/><b>PATHOLOGIST</b></td>
        </tr>
    </table>
</htmlpagefooter>
...
mpdf-->
<div id="main">
    <table width=95% cellpadding=0 cellspacing=0 align=center style="border-collapse: collapse; font-size: 10pt;">
		<tr>
			<td width=25% class="itemHeader" style="text-align:left;">TEST</td>
			<td width=25% class="itemHeader" style="text-align:center;">RESULT</td>
			<td width=25% class="itemHeader" style="text-align:center;">FLAG</td>
			<td width=25% class="itemHeader" style="text-align:right;">NORMAL REFERENCE</td>
		</tr>
		<tr><td height=20></td></tr>
		<tr>
			<td >'.$b['procedure'].'</td>
			<td style="text-align:center;">'.$b['value'].' '.$b['unit'].'</td>
			<td style="text-align:center;">'.$flag.'</td>
			<td style="text-align:center;">'.$attr.'</td>
		</tr>
		<tr><td height=45></td></tr>
    </table>
	<table width=95% align=center style="margin-top: 5px; font-size: 9pt; font-style: italic;">
		<tr>
