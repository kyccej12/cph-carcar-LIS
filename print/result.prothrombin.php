...
/* MYSQL QUERIES SECTION */
    ...
    $b = $con->getArray("select * from lab_prothrombin where enccode = '$order[enccode]' and `code` = '$order[code]' and serialno = '$order[serialno]';");
	...
	if($b['seconds'] > 14) { $flag1 = 'H'; }
	if($b['seconds'] < 10) { $flag1 = 'L'; }

	if($b['percent'] > 1.30) { $flag2 = 'H'; }
	if($b['percent'] < 0.07) { $flag2 = 'L'; }

	if($b['inr'] > 1.30) { $flag3 = 'H'; }
	if($b['inr'] < 0.07) { $flag3 = 'L'; }
...
if($b['verified'] != 'Y') {
	...
	$mpdf->SetWatermarkImage ('../images/logosmall.jpg',0.2,'F','P');
...
<table width=100% cellpadding=2 cellspacing=0 style="font-size: 10pt;margin-top:20px;">
	...
	<tr>
	<td width=100% colspan=4 style="background-color: #8db4e2; border-top: 1px solid black; padding: 5px; font-weight: bold;" align=center>PROTHROMBIN TIME</td>
	</tr>  
</table>
...
mpdf-->
<div id="main">
    <table width=95% cellpadding=0 cellspacing=0 align=center style="border-collapse: collapse; font-size: 9pt;">
		<tr>
			...
			<td width=20% class="itemHeader">UNIT</td>
		</tr>
		<tr>
			<td class="itemResult" align=left>PROTHROMBIN TIME</td>
			<td class="itemResult">'.$b['seconds'].'</td>
			<td class="itemResult" style="font-weight: bold; color: red;">'.$flag1.'</td>
			<td class="itemResult">10 - 14</td>
			<td class="itemResult">seconds</td>
		</tr>
		<tr>
			<td class="itemResult" align=left></td>
			<td class="itemResult">'.$b['percent'].'</td>
			<td class="itemResult" style="font-weight: bold; color: red;">'.$flag2.'</td>
			<td class="itemResult">0.07 - 1.30</td>
			<td class="itemResult">%</td>
		</tr>
		<tr>
			<td class="itemResult" align=left></td>
			<td class="itemResult">'.$b['inr'].'</td>
			<td class="itemResult" style="font-weight: bold; color: red;">'.$flag3.'</td>
			<td class="itemResult">0.7 - 1.3</td>
			<td class="itemResult">INR</td>
		</tr>
    </table>
