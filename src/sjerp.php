	...
	switch($_POST['mod']) {
		...
		case "retrieveOrderForSample":
			...
			//if($_SESSION['userid'] == 1) {
				//echo "select count(*) as samplecount from (SELECT a.proccode, b.procdesc FROM hospital_dbo.hdocord a LEFT JOIN hospital_dbo.hprocm b ON a.proccode = b.proccode LEFT JOIN cph_carcar.services_master c ON a.proccode = c.ihomis_code WHERE a.enccode = '$a[enccode]' AND c.container_type = '$d[container_type]' and c.sample_type = '$d[sample_type]') a;";
		...
		case "retrieveSameSampleForPrint":
			...
			$cat = $con->getArray("select category,subcategory,sample_type,container_type from services_master where ihomis_code = '$_POST[proccode]';");
			$sQuery = $con->dbquery("SELECT a.proccode, b.procdesc, c.code FROM hospital_dbo.hdocord a LEFT JOIN hospital_dbo.hprocm b ON a.proccode = b.proccode LEFT JOIN cph_carcar.services_master c ON a.proccode = c.ihomis_code WHERE a.enccode = '$_POST[enccode]' AND c.container_type = '$cat[container_type]' and c.sample_type = '$cat[sample_type]' and c.category = '$cat[category]' and c.subcategory = '$cat[subcategory]' and a.dotime = '$_POST[dotime]' group by a.proccode;");
		...
		case "saveSample":
			...
			if($isE > 0) {
				...
				$con->dbquery("UPDATE IGNORE hospital_dbo.hdocord set estatus = 'P' where enccode = '$_POST[phleb_sodetails]' and dotime = '$_POST[phleb_dotime]' and proccode = '$_POST[phleb_code]';");

				/* Create Log (No data structure where to put audit trail when a record in hdocord is update) */
				$con->dbquery("INSERT IGNORE into hdocord_log (enccode,dotime,proccode,estatus_update,updated_by,updated_on) values ('$_POST[phleb_sodetails]','$_POST[phleb_dotime]','$_POST[phleb_code]','P','$_SESSION[userid]',now());");
			...
			if(count($_POST['othercodes']) > 0) {
				foreach($_POST['othercodes'] as $scode) {
					...
					if($isE > 0) {
						...
						/* Update as per recommendation by DoH/ICTO to update hdocord everytime a sample is taken */
						$con->dbquery("UPDATE IGNORE hospital_dbo.hdocord set estatus = 'P' where enccode = '$_POST[phleb_sodetails]' and dotime = '$_POST[phleb_dotime]' and proccode = '$scode';");

						/* Create Log (No data structure where to put audit trail when a record in hdocord is update) */
						$con->dbquery("INSERT IGNORE into hdocord_log (enccode,dotime,proccode,estatus_update,updated_by,updated_on) values ('$_POST[phleb_sodetails]','$_POST[phleb_dotime]','$scode','P','$_SESSION[userid]',now());");
		...
		case "retrieveSample":
			echo json_encode($con->getArray("SELECT patientname AS pname,`code`,`procedure`,sampletype,serialno, location, DATE_FORMAT(extractdate,'%m/%d/%Y') AS exdate,  SUBSTR(extractime,1,2) AS hr, SUBSTR(extractime,4,2) AS MIN, extractby FROM lab_samples WHERE record_id = '$_POST[lid]';"));
		break;

		case "rejectSample":
			$con->dbquery("UPDATE IGNORE lab_samples set status = '2', rejection_remarks = '".$con->escapeString(htmlentities($_POST['phleb_remarks']))."', updated_by = '$uid', updated_on = now() where record_id = '$_POST[lid]';");
			//echo "UPDATE IGNORE lab_samples set status = '2', rejection_remarks = '".$con->escapeString(htmlentities($_POST['phleb_remarks']))."', updated_by = '$uid', updated_on = now() where record_id = '$_POST[lid]';";
		break;
		...
		case "resultSingle":
			...
			switch($_POST['submod']) {
				...
				break;
				case "ogtt":
					$b = $con->getArray("select result, result_urine, result2, result2_urine, result3, result3_urine, remarks from lab_ogtt where enccode = '$order[enccode]' and serialno = '$order[serialno]' and code = '$order[code]';");
				break;
				case "prothrombinRes":
				...
				break;
				case "typhoid":
					$b = $con->getArray("select typhoid_igm, typhoid_igg, remarks from lab_typhoid where enccode = '$order[enccode]' and serialno = '$order[serialno]' and code = '$order[code]';");
				break;
		...
		case "saveTyphoid":
			list($cnt) = $con->getArray("select count(*) from lab_typhoid where enccode = '$_POST[typhoid_enccode]' and serialno = '$_POST[typhoid_serialno]';");
			if($cnt > 0) {
				$con->dbquery("UPDATE IGNORE lab_typhoid set result_date = '".$con->formatDate($_POST['typhoid_date'])."', typhoid_igm = '$_POST[typhoid_igm]', typhoid_igg = '$_POST[typhoid_igg]', remarks = '".$con->escapeString($_POST['typhoid_remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[typhoid_enccode]' and code = '$_POST[typhoid_code]' and serialno = '$_POST[typhoid_serialno]';");
			} else {
				$con->dbquery("INSERT IGNORE INTO lab_typhoid (enccode,result_date,sampletype,serialno,code,typhoid_igm,typhoid_igg,performed_by,remarks,created_by,created_on) VALUES ('$_POST[typhoid_enccode]','".$con->formatDate($_POST['typhoid_date'])."','$_POST[typhoid_spectype]','$_POST[typhoid_serialno]','$_POST[typhoid_code]','$_POST[typhoid_igm]','$_POST[typhoid_igg]','$_POST[typhoid_result_by]','".$con->escapeString($_POST['typhoid_remarks'])."','$uid',now());");
			}
			$con->updateLabSampleStatus($_POST['typhoid_enccode'],$_POST['typhoid_code'],$_POST['typhoid_serialno'],'3',$uid);
		break;

		case "validateTyphoid":
			$con->dbquery("UPDATE IGNORE lab_typhoid set result_date = '".$con->formatDate($_POST['typhoid_date'])."', typhoid_igm = '$_POST[typhoid_igm]', typhoid_igg = '$_POST[typhoid_igg]', remarks = '".$con->escapeString($_POST['typhoid_remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[typhoid_enccode]' and code = '$_POST[typhoid_code]' and serialno = '$_POST[typhoid_serialno]';");
			$con->validateResult("lab_typhoid",$_POST['typhoid_enccode'],$_POST['typhoid_code'],$_POST['typhoid_serialno'],$uid);
			$con->updateLabSampleStatus($_POST['typhoid_enccode'],$_POST['typhoid_code'],$_POST['typhoid_serialno'],'4',$uid);	
		break;
		...
		/* ABG result */
		case "saveAbgResult":
			list($cnt) = $con->getArray("select count(*) from lab_abgresult where enccode = '$_POST[abg_enccode]' and serialno = '$_POST[abg_serialno]';");
			if($cnt > 0) {
				$con->dbquery("UPDATE IGNORE lab_abgresult set result_date = '".$con->formatDate($_POST['abg_date'])."', ph = '$_POST[abg_ph]', pco2 = '$_POST[abg_pco2]', po2 = '$_POST[abg_po2]', thb = '$_POST[abg_thb]', so2 = '$_POST[abg_so2]', be = '$_POST[abg_be]', hco3 = '$_POST[abg_hco3]', hctc = '$_POST[abg_hctc]', temp = '$_POST[abg_temp]', remarks = '".$con->escapeString($_POST['abg_remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[abg_enccode]' and code = '$_POST[abg_ihomis_code]' and serialno = '$_POST[abg_serialno]';");
			} else {
				$con->dbquery("INSERT IGNORE INTO lab_abgresult (enccode,result_date,sampletype,serialno,code,ph,pco2,po2,thb,so2,be,hco3,hctc,temp,remarks,created_by,created_on) VALUES ('$_POST[abg_enccode]','".$con->formatDate($_POST['abg_date'])."','$_POST[abg_spectype]','$_POST[abg_serialno]','$_POST[abg_ihomis_code]','$_POST[abg_ph]','$_POST[abg_pco2]','$_POST[abg_po2]','$_POST[abg_thb]','$_POST[abg_so2]','$_POST[abg_be]','$_POST[abg_hco3]','$_POST[abg_hctc]','$_POST[abg_temp]','".$con->escapeString($_POST['abg_remarks'])."','$uid',now());");
			}
			$con->updateLabSampleStatus($_POST['abg_enccode'],$_POST['abg_ihomis_code'],$_POST['abg_serialno'],'3',$uid);
		break;

		case "validateAbgResult":
			$con->dbquery("UPDATE IGNORE lab_abgresult set result_date = '".$con->formatDate($_POST['abg_date'])."', ph = '$_POST[abg_ph]', pco2 = '$_POST[abg_pco2]', po2 = '$_POST[abg_po2]', thb = '$_POST[abg_thb]', so2 = '$_POST[abg_so2]', be = '$_POST[abg_be]', hco3 = '$_POST[abg_hco3]', hctc = '$_POST[abg_hctc]', temp = '$_POST[abg_temp]', remarks = '".$con->escapeString($_POST['abg_remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[abg_enccode]' and code = '$_POST[abg_ihomis_code]' and serialno = '$_POST[abg_serialno]';");
			$con->validateResult("lab_abgresult",$_POST['abg_enccode'],$_POST['abg_ihomis_code'],$_POST['abg_serialno'],$uid);
			$con->updateLabSampleStatus($_POST['abg_enccode'],$_POST['abg_ihomis_code'],$_POST['abg_serialno'],'4',$uid);	
		break;

		/* Ketone*/
		case "saveKetoneResult":
			list($cnt) = $con->getArray("select count(*) from lab_ketone where enccode = '$_POST[ketone_enccode]' and serialno = '$_POST[ketone_serialno]';");
			if($cnt > 0) {
				$con->dbquery("UPDATE IGNORE lab_ketone set result_date = '".$con->formatDate($_POST['ketone_date'])."', color = '$_POST[color]', transparency = '$_POST[transparency]', ketone = '$_POST[ketone]', time_collected = '$_POST[time_collected]', remarks = '".$con->escapeString($_POST['remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[ketone_enccode]' and code = '$_POST[ketone_ihomis_code]' and serialno = '$_POST[ketone_serialno]';");
			} else {
				$con->dbquery("INSERT IGNORE INTO lab_ketone (enccode,result_date,sampletype,serialno,code,color,transparency,ketone,time_collected,remarks,created_by,created_on) VALUES ('$_POST[ketone_enccode]','".$con->formatDate($_POST['ketone_date'])."','$_POST[ketone_spectype]','$_POST[ketone_serialno]','$_POST[ketone_ihomis_code]','$_POST[color]','$_POST[transparency]','$_POST[ketone]','$_POST[time_collected]','".$con->escapeString($_POST['remarks'])."','$uid',now());");
			}
			$con->updateLabSampleStatus($_POST['ketone_enccode'],$_POST['ketone_ihomis_code'],$_POST['ketone_serialno'],'3',$uid);
		break;

		case "validateKetoneResult":
			$con->dbquery("UPDATE IGNORE lab_ketone set result_date = '".$con->formatDate($_POST['ketone_date'])."', color = '$_POST[color]', transparency = '$_POST[transparency]', ketone = '$_POST[ketone]', time_collected = '$_POST[time_collected]', remarks = '".$con->escapeString($_POST['remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[ketone_enccode]' and code = '$_POST[ketone_ihomis_code]' and serialno = '$_POST[ketone_serialno]';");
			$con->validateResult("lab_ketone",$_POST['ketone_enccode'],$_POST['ketone_ihomis_code'],$_POST['ketone_serialno'],$uid);
			$con->updateLabSampleStatus($_POST['ketone_enccode'],$_POST['ketone_ihomis_code'],$_POST['ketone_serialno'],'4',$uid);	
		break;

		case "saveCrossMatching":
			list($cnt) = $con->getArray("select count(*) from lab_crossmatching where enccode = '$_POST[cmr_enccode]' and serialno = '$_POST[cmr_serialno]';");
			if($cnt > 0) {
				$con->dbquery("UPDATE IGNORE lab_crossmatching set result_date = '".$con->formatDate($_POST['cmr_date'])."', cmr_group = '$_POST[cmr_group]', cmr_group_pos = '$_POST[cmr_group_pos]', cmr_examination = '$_POST[cmr_examination]', cmr_donor_btype1 = '$_POST[cmr_donor_btype1]', cmr_donor_btype2 = '$_POST[cmr_donor_btype2]', cmr_rh_ag1 = '$_POST[cmr_rh_ag1]', cmr_rh_ag2 = '$_POST[cmr_rh_ag2]', cmr_bld_comp1 = '$_POST[cmr_bld_comp1]', cmr_bld_comp2 = '$_POST[cmr_bld_comp2]', cmr_serialno1 = '$_POST[cmr_serialno1]', cmr_serialno2 = '$_POST[cmr_serialno2]', cmr_segment1 = '$_POST[cmr_segment1]', cmr_segment2 = '$_POST[cmr_segment2]', cmr_expirydate1 = '".$con->formatDate($_POST['cmr_expirydate1'])."', cmr_expirydate2 = '".$con->formatDate($_POST['cmr_expirydate2'])."', cmr_compatibility_res1 = '$_POST[cmr_compatibility_res1]', cmr_compatibility_res2 = '$_POST[cmr_compatibility_res2]', cmr_remarks = '".$con->escapeString($_POST['cmr_remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[cmr_enccode]' and code = '$_POST[cmr_ihomis_code]' and serialno = '$_POST[cmr_serialno]';");
			} else {
				$con->dbquery("INSERT IGNORE INTO lab_crossmatching (enccode,result_date,sampletype,serialno,code,cmr_group,cmr_group_pos,cmr_examination,cmr_donor_btype1,cmr_donor_btype2,cmr_rh_ag1,cmr_rh_ag2,cmr_bld_comp1,cmr_bld_comp2,cmr_serialno1,cmr_serialno2,cmr_segment1,cmr_segment2,cmr_expirydate1,cmr_expirydate2,cmr_compatibility_res1,cmr_compatibility_res2,cmr_remarks,created_by,created_on) VALUES ('$_POST[cmr_enccode]','".$con->formatDate($_POST['cmr_date'])."','$_POST[cmr_sampletype]','$_POST[cmr_serialno]','$_POST[cmr_ihomis_code]','$_POST[cmr_group]','$_POST[cmr_group_pos]','$_POST[cmr_examination]','$_POST[cmr_donor_btype1]','$_POST[cmr_donor_btype2]','$_POST[cmr_rh_ag1]','$_POST[cmr_rh_ag2]','$_POST[cmr_bld_comp1]','$_POST[cmr_bld_comp2]','$_POST[cmr_serialno1]','$_POST[cmr_serialno2]','$_POST[cmr_segment1]','$_POST[cmr_segment2]','".$con->formatDate($_POST['cmr_expirydate1'])."','".$con->formatDate($_POST['cmr_expirydate2'])."','$_POST[cmr_compatibility_res1]','$_POST[cmr_compatibility_res2]','".$con->escapeString($_POST['cmr_remarks'])."','$uid',now());");
			}
			$con->updateLabSampleStatus($_POST['cmr_enccode'],$_POST['cmr_ihomis_code'],$_POST['cmr_serialno'],'3',$uid);
		break;

		case "validateCrossMatching":
			$con->dbquery("UPDATE IGNORE lab_crossmatching set result_date = '".$con->formatDate($_POST['cmr_date'])."', cmr_group = '$_POST[cmr_group]', cmr_group_pos = '$_POST[cmr_group_pos]', cmr_examination = '$_POST[cmr_examination]', cmr_donor_btype1 = '$_POST[cmr_donor_btype1]', cmr_donor_btype2 = '$_POST[cmr_donor_btype2]', cmr_rh_ag1 = '$_POST[cmr_rh_ag1]', cmr_rh_ag2 = '$_POST[cmr_rh_ag2]', cmr_bld_comp1 = '$_POST[cmr_bld_comp1]', cmr_bld_comp2 = '$_POST[cmr_bld_comp2]', cmr_serialno1 = '$_POST[cmr_serialno1]', cmr_serialno2 = '$_POST[cmr_serialno2]', cmr_segment1 = '$_POST[cmr_segment1]', cmr_segment2 = '$_POST[cmr_segment2]', cmr_expirydate1 = '".$con->formatDate($_POST['cmr_expirydate1'])."', cmr_expirydate2 = '".$con->formatDate($_POST['cmr_expirydate2'])."', cmr_compatibility_res1 = '$_POST[cmr_compatibility_res1]', cmr_compatibility_res2 = '$_POST[cmr_compatibility_res2]', cmr_remarks = '".$con->escapeString($_POST['cmr_remarks']) . "', updated_by = '$uid', updated_on = now() where enccode = '$_POST[cmr_enccode]' and code = '$_POST[cmr_ihomis_code]' and serialno = '$_POST[cmr_serialno]';");
			$con->validateResult("lab_crossmatching",$_POST['cmr_enccode'],$_POST['cmr_ihomis_code'],$_POST['cmr_serialno'],$uid);
			$con->updateLabSampleStatus($_POST['cmr_enccode'],$_POST['cmr_ihomis_code'],$_POST['cmr_serialno'],'4',$uid);	
		break;

		case "saveOGTT":
			list($cnt) = $con->getArray("select count(*) from lab_ogtt where enccode = '$_POST[ogtt_enccode]' and serialno = '$_POST[ogtt_serialno]';");
			if($cnt > 0) {
				$con->dbquery("UPDATE IGNORE lab_ogtt set result_date = '".$con->formatDate($_POST['ogtt_sodate'])."', result = '$_POST[ogtt_fbs]', result_urine = '$_POST[ogtt_fbs_urine]', result2 = '$_POST[ogtt_fhours]', result2_urine = '$_POST[ogtt_result2_urine]', result3 = '$_POST[ogtt_2hours]', result3_urine = '$_POST[ogtt_result3_urine]', performed_by = '$_POST[ogtt_result_by]', remarks = '".$con->escapeString($_POST['ogtt_remarks']) . "' where enccode = '$_POST[ogtt_enccode]' and code = '$_POST[ogtt_code]' and serialno = '$_POST[ogtt_serialno]';");
			} else {
				$con->dbquery("INSERT IGNORE INTO lab_ogtt (enccode,result_date,sampletype,serialno,code,result,result_urine,result2,result2_urine,result3,result3_urine,performed_by,remarks,created_by,created_on) VALUES ('$_POST[ogtt_enccode]','".$con->formatDate($_POST['ogtt_date'])."','$_POST[ogtt_spectype]','$_POST[ogtt_serialno]','$_POST[ogtt_code]','$_POST[ogtt_fbs]','$_POST[ogtt_fbs_urine]','$_POST[ogtt_fhours]','$_POST[ogtt_fhours_urine]','$_POST[ogtt_2hours]','$_POST[ogtt_2hours_urine]','$_POST[ogtt_result_by]','".$con->escapeString($_POST['ogtt_remarks'])."','$uid',now());");
			}
		...
		case "saveCoagulationResult":
			...
			$con->updateLabSampleStatus($_POST['coagulation_enccode'],'LABOR00181',$_POST['coagulation_serialno'],'3',$uid);
			$con->updateLabSampleStatus($_POST['coagulation_enccode'],'LABOR3002',$_POST['coagulation_serialno'],'3',$uid);
		break;

		case "validateCoagulationResult":
			$con->dbquery("UPDATE lab_coagulation set proth_seconds = '$_POST[proth_seconds]', proth_percent = '$_POST[proth_percent]', proth_inr = '$_POST[proth_inr]', aptt_seconds = '$_POST[aptt_seconds]', remarks = '".$con->escapeString($_POST['remarks']) . "' where enccode = '$_POST[coagulation_enccode]' and serialno = '$_POST[coagulation_serialno]';");
			$con->validateResult("lab_coagulation",$_POST['coagulation_enccode'],$_POST['coagulation_proccode'],$_POST['coagulation_serialno'],$uid);
			$con->updateLabSampleStatus($_POST['coagulation_enccode'],'LABOR00181',$_POST['coagulation_serialno'],'4',$uid);	
			$con->updateLabSampleStatus($_POST['coagulation_enccode'],'LABOR3002',$_POST['coagulation_serialno'],'4',$uid);	
		break;
		...
		case "validateBloodChem":
			...
			$con->dbquery("UPDATE IGNORE lab_bloodchem SET result_date = '".$con->formatDate($_POST['bloodchem_date'])."',glucose='".$con->formatDigit($_POST['glucose'])."',uric = '".$con->formatDigit($_POST['uric'])."',bun = '".$con->formatDigit($_POST['bun'])."',creatinine = '".$con->formatDigit($_POST['creatinine'])."', ldh = '".$con->formatDigit($_POST['ldh'])."', cholesterol = '".$con->formatDigit($_POST['cholesterol'])."',triglycerides = '".$con->formatDigit($_POST['triglycerides'])."',hdl = '".$con->formatDigit($_POST['hdl'])."',ldl = '".$con->formatDigit($_POST['ldl'])."',vldl = '".$con->formatDigit($_POST['vldl'])."',sgot = '".$con->formatDigit($_POST['sgot'])."',sgpt = '".$con->formatDigit($_POST['sgpt'])."',alkaline = '".$con->formatDigit($_POST['alkaline'])."',bilirubin = '".$con->formatDigit($_POST['bilirubin'])."',bilirubin_direct = '".$con->formatDigit($_POST['bilirubin_direct'])."',bilirubin_indirect = '".$con->formatDigit($_POST['bilirubin_indirect'])."',protein = '".$con->formatDigit($_POST['protein'])."',albumin = '".$con->formatDigit($_POST['albumin'])."',globulin = '".$con->formatDigit($_POST['globulin'])."',agratio = '".$con->formatDigit($_POST['agratio'])."',electrolytes_na = '".$con->formatDigit($_POST['electrolytes_na'])."',electrolytes_k = '".$con->formatDigit($_POST['electrolytes_k'])."',electrolytes_ci = '".$con->formatDigit($_POST['electrolytes_ci'])."',calcium = '".$con->formatDigit($_POST['calcium'])."',total_calcium = '".$con->formatDigit($_POST['total_calcium'])."',phosphorus = '".$con->formatDigit($_POST['phosphorus'])."',ggt = '".$con->formatDigit($_POST['ggt'])."', ion_calcium = '".$con->formatDigit($_POST['ion_calcium'])."', troponin = '".$con->formatDigit($_POST['troponin'])."', cbg = '".$con->formatDigit($_POST['cbg'])."', rbs = '$_POST[rbs]', magnesium = '$_POST[magnesium]', remarks = '".$con->escapeString(htmlentities($_POST['remarks']))."',updated_by = '$uid',updated_on = NOW() where enccode = '$_POST[bloodchem_enccode]' and serialno = '$_POST[bloodchem_serialno]';");
		...
		case "resultRestore":
			$con->dbquery("update lab_samples set status = '1' where enccode = '$_POST[enccode]' and serialno = '$_POST[serialno]';");
		break;
		...
		case "requestChecked":
			...
			if($_POST['toggle'] == 'Y') {
				...
				$a = $con->getArray("SELECT DATE_FORMAT(dodate,'%Y-%m-%d') AS xorderdate, concat(trim(c.patlast),',',trim(c.patfirst),',',ifnull(trim(c.patmiddle),'')) as patname, DATE_FORMAT(c.patbdate,'%Y-%m-%d') AS xbday FROM hospital_dbo.hdocord a LEFT JOIN hospital_dbo.hprocm b ON a.proccode = b.proccode LEFT JOIN hospital_dbo.hperson c ON a.hpercode = c.hpercode WHERE a.enccode = '$enccode' and a.proccode = '$proccode' and a.dodate = '$dotime';");
				$con->calculateAge($a['xorderdate'],$a['xbday']);
		
				$con->dbquery("insert into tmp_barcoder (uid,enccode,orderdate,hpercode,hpatroom,patname,patbday,patage,patgender,proccode,`code`,description,sampletype,containertype,dotime,dopriority,subcategory,physician) values ('$_SESSION[userid]','$enccode','$a[xorderdate]','$hmrno','$room','$a[patname]','$a[xbday]','".$con->ageDisplay."','$patsex','$proccode','$code','".$con->escapeString($desc)."','$stype','$ctype','$dotime','$stat','$subcat','".$con->escapeString($provider)."');");
		...
		break;
		case "checkStatPing":
			list($statCount) = $con->getArray("SELECT count(serialno) FROM lab_samples WHERE dopriority = 'STAT' and status = '1' AND dotime >= '2022-10-31' ORDER BY dotime DESC LIMIT 1;");
			echo $statCount;
		break;
		case "setPinged":
			//$con->dbquery("update lab_samples set stat_pinged = 'Y' where enccode = '$_POST[enccode]' and dotime = '$_POST[dotime]' and `code` = '$_POST[code]' and dopriority = 'STAT';");
		break;
	}
?>