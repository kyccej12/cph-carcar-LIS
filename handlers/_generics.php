<?php
	...
	class _init extends myDB {
		...
		function identRoom($enccode) {
			list($room) = parent::getArray("SELECT CONCAT(e.wardname,' - ',rmname,' - ',bdname) FROM lab_samples a LEFT JOIN hospital_dbo.hpatroom b ON a.enccode = b.enccode LEFT JOIN hospital_dbo.hroom c ON b.rmintkey = c.rmintkey LEFT JOIN hospital_dbo.hbed d ON b.bdintkey = d.bdintkey LEFT JOIN hospital_dbo.hward e ON b.wardcode = e.wardcode WHERE a.enccode = '$enccode' LIMIT 1,2;");
			return $room;
		}
		...
		function getAttribute($code,$age,$gender) {
			...
			/* ABG */
			if($code == 'BE' || $code == 'HCO3' || $code == 'HCTC' ) {
				if($att['min_value'] == 0) {
		...
		function updateLabSampleStatus($enccode,$code,$sn,$stat,$uid) {
			...
			echo "update ignore lab_samples set status = '$stat', updated_by = '$uid', updated_on = now() where enccode = '$enccode' and serialno = '$sn' $whereString;";
		}
